...
//    private AtomicInteger currentType = new AtomicInteger(0);//当前同步的类型
...
//        BackgroundThreadManager.getInstance().addWriteData(ContextUtil.app, realHealthData);
//
//        if (isSync) {
...
//        //一个一个写，设备上来的数据才不会错乱
//        if(currentType.get() < typeArray.size()){
//            byte[] indexTab = ProtoBufSendBluetoothCmdImpl.getInstance().itHisData(typeArray.get(currentType.get()));
//            BackgroundThreadManager.getInstance().addWriteData(ContextUtil.app, indexTab);
//        }
//    }
//
////    private void
...
//        if (positionSync.get() < index_80s.size()) {
//            int hisDataType = index_80s.get(positionSync.get()).getIndexType();
//            mHandler.removeCallbacks(syncTimeOutRunnable);
//            mHandler.postDelayed(syncTimeOutRunnable,15*1000);
//            ProtoBuf_index_80 index = index_80s.get(positionSync.get());
//            final int startSeq = index.getStart_idx();
//            final int endSeq = index.getEnd_idx();
//            oneDayHasFinish = false;
//            lastPosition = -1;
//            ProtoBufDataParsePresenter.CURRENT_INDEX_TABLE = new DateUtil(index.getYear(),index.getMonth(),index.getDay()).getSyyyyMMddDate() + hisDataType;
//            AwLog.i(Author.GuanFengJun,"同步"+index.getMonth()+"-"+index.getDay()+" == "+startSeq+"-"+endSeq + "==hisDataType--"+ hisDataType);
...
//        BackgroundThreadManager.getInstance().addWriteData(ContextUtil.app, hisData);
//        nowSyncType = type;
//    }
//
...
//                    ProtoBufUpdate.getInstance().startUpdateGps(ProtoBufUpdate.Type.TYPE_GPS);
//                }
//                if(typeArray.contains(RRI_DATA)){
...
////        int currentIndex = -1;
//        if(index_80s==null || positionSync.get() >= index_80s.size()){
//            return 0;
//        }
//        if(nowSyncType!=type){
...
//        }
//
//        int totalNum = index_80s.get(positionSync.get()).getEnd_idx() - index_80s.get(positionSync.get()).getStart_idx();
//        int progress = (seq+1-index_80s.get(positionSync.get()).getStart_idx())*100/totalNum;
...
//                typeDesc = "RRI";
//            }
//            int postProgress = Math.min(progress, 100);
//            EventBus.getDefault().post(new SyncDataEvent(postProgress, false, index_80s.size(), positionSync.get() + 1, typeDesc));
...
//        Locale locale = ContextUtil.app.getResources().getConfiguration().locale;
...
//            index_table.saveOrUpdate("uid=? and dataFrom =? and date=?",
//                    String.valueOf(dbIndex.getUid()), dbIndex.getData_from(), d.getY_M_D_H_M_S());
//        }
//    }
//
...
//        if(index_80s!=null && index_80s.size()>0 && (positionSync.get() < index_80s.size())) {
//            oneDayHasFinish = true;
//            ProtoBuf_index_80 index80 = index_80s.get(positionSync.get());
